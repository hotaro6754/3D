...
const _materials = new Set<THREE.ShaderMaterial>();
...
function smoothedGeometry(geo: THREE.BufferGeometry): THREE.BufferGeometry {
 ...
 */
export function hullOutline(mesh: THREE.Mesh | THREE.InstancedMesh, { thickness = 0.0038, color = PAL.ink, opacity = 1 }: OutlineOptions = {}): THREE.Mesh | THREE.InstancedMesh | null {
  ...
  let shell: THREE.Mesh | THREE.InstancedMesh;
  if ((mesh as THREE.InstancedMesh).isInstancedMesh) {
    const instMesh = mesh as THREE.InstancedMesh;
    shell = new THREE.InstancedMesh(geo, mat, instMesh.count);
    (shell as THREE.InstancedMesh).instanceMatrix = instMesh.instanceMatrix;
    shell.count = instMesh.count;
  } else {
...
export function hullOutlineTree<T extends THREE.Object3D>(root: T, opts: OutlineOptions = {}): T {
  const targets: (THREE.Mesh | THREE.InstancedMesh)[] = [];
