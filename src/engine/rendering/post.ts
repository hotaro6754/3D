...
const GRADE_SHADER = {
  uniforms: {
    tDiffuse: { value: null },
    uShadowTint: { value: new THREE.Color(0x98a1c9) }, // Cooler shadows
    uLightTint: { value: new THREE.Color(0xffe6c2) }, // Warmer highlights
...
function makeQuad(def: any): ShaderQuad {
...
export class Pipeline {
  ...
  grade: ShaderQuad;
  fxaa: ShaderQuad;
  enabled: { ink: boolean; grade: boolean; fxaa: boolean };
  scale?: number;
  forceScale?: number;

  constructor(renderer: THREE.WebGLRenderer, scene: THREE.Scene, camera: THREE.Camera, { pixelBudget = 4.6e6 }: PipelineOptions = {}) {
    ...
    const opts = {
      ...
      depthBuffer: true,
      stencilBuffer: false,
      colorSpace: THREE.NoColorSpace as any,
    };
  ...
  setSize(w: number, h: number): void {
    ...
    this.ink.mat.uniforms.uNear.value = (this.camera as THREE.PerspectiveCamera).near || 0.25;
    this.ink.mat.uniforms.uFar.value = (this.camera as THREE.PerspectiveCamera).far || 600;
