...
function make(w: number, h: number, draw: (c: CanvasRenderingContext2D, w: number, h: number) => void, { srgb = true, repeat = null, aniso = 4 }: MakeOptions = {}): THREE.CanvasTexture {
...
function fitText(c: CanvasRenderingContext2D, text: string, maxW: number, size: number, font = JP_FONT, weight = 'bold'): number {
  let s = size;
  do {
...
function centered(c: CanvasRenderingContext2D, text: string, x: number, y: number, maxW: number, size: number, color: string, weight = 'bold', spacing = 0): number {
...
function vertical(c: CanvasRenderingContext2D, text: string, x: number, y0: number, step: number, size: number, color: string): void {
...
export const poster = (variant = 0) =>
  cached('poster' + variant, () =>
    make(320, 448, (c, w, h) => {
      const sets = [
        { bg: '#fdf7e8', bar: PAL.red, t: 'DATA', s: 'INDEX' },
        { bg: '#eef6fd', bar: PAL.blue, t: 'CODE', s: 'SYSTEM' },
        { bg: '#fdeef1', bar: PAL.purple, t: 'DEV', s: 'NODE' },
        { bg: '#f4fbef', bar: PAL.leafDeep, t: 'WEB', s: 'BUILD' },
      ];
...
export const vendHeader = (variant = 0) =>
  cached('vendHeader' + variant, () =>
    make(512, 160, (c, w, h) => {
      const sets = [
        { bg: '#ffffff', fg: PAL.red, t: 'SYS_01' },
        { bg: '#e0453f', fg: 0xffffff, t: 'ENERGY' },
        { bg: '#2e9a98', fg: 0xffffff, t: 'WATER' },
      ];
      const st = sets[variant % sets.length];
      c.fillStyle = st.bg;
      c.fillRect(0, 0, w, h);
...
export const specialRoomTex = (variant = 0) =>
  cached('specialRoom' + variant, () =>
    make(512, 256, (c, w, h) => {
      ...
      c.fillStyle = '#e2e0d8';
      c.fillRect(0, 0, w, 16);
      const box = (x: number, y: number, bw: number, bh: number, col: string) => { c.fillStyle = col; c.fillRect(x, y, bw, bh); };
      if (v === 0) {
        for (let r = 0; r < 2; r++) {
