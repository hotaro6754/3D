...
const rampCache = new Map<string | number, THREE.DataTexture>();

export function gradientMap(bands: string | number = 3): THREE.DataTexture {
  const key = bands;
...
const TOON_PATCH = `
	...
	vec3 irradiance = mix( uShadowTint, vec3( 1.0 ), celBand ) * directLight.color;`;
...
function applyShadowTint(mat: THREE.MeshToonMaterial, tint: number): THREE.MeshToonMaterial {
...
export function cel(opts: CelOptions = {}): THREE.MeshToonMaterial {
  const {
    color = 0xffffff,
    bands = 3,
    tint = 0x6c5f8c, // Original Sakura Crossing cool-violet shadow tint
    flat = true,
    map = null,
    emissive = null,
    emissiveIntensity = 1,
  ...
  const key = cache && !map && !alphaMap
    ...
    : null;
  if (key && matCache.has(key)) return matCache.get(key) as THREE.MeshToonMaterial;
...
const flatCache = new Map<string, THREE.Material>();
...
export function flat(opts: FlatOptions = {}): THREE.MeshBasicMaterial {
  const {
    color = 0xffffff,
    map = null,
    transparent = false,
  ...
  const key = cache && !map
    ...
    : null;
  if (key && flatCache.has(key)) return flatCache.get(key) as THREE.MeshBasicMaterial;
