...
export function makeHouse(o) {
  ...
  function placeFront(u, y, ww, wh, kind = 'window') {
    ...
    // Create an independent door mesh for animation
    const doorGeo = new THREE.BoxGeometry(dx, wh, dz);
    const doorMesh = new THREE.Mesh(doorGeo, m.door);
    // Position it appropriately
    doorMesh.position.set(px + fx * 0.02, y, pz + fz * 0.02);
    // Set pivot to the edge so it swings open properly.
    if (frontIsX) {
      doorGeo.translate(0, 0, dz / 2);
      doorMesh.position.z -= dz / 2;
    } else {
      doorGeo.translate(dx / 2, 0, 0);
      doorMesh.position.x -= dx / 2;
    }
    doorMesh.castShadow = true;
    doorMesh.receiveShadow = true;
    doorMesh.name = 'door_hinge';
    g.add(doorMesh);
...
export function buildBuildings(ctx: PortfolioContext) {
  const y = 0.03;
  const floorH = 2.72;

  for (const key of Object.keys(WORLD_LANDMARKS)) {
    const lm = WORLD_LANDMARKS[key];
    if (lm.id === 'bridge') continue;
    const bldg = makeHouse({ ...lm, y });
    bldg.name = lm.id;
    ctx.scene.add(bldg);

    // Register Collision - Hollow walls so player can enter
    const hx = lm.w / 2;
    const hz = lm.d / 2;
    ...
    // Initialize door collider immediately so closed doors block passage
    ...
    // We place the interaction point exactly at the door
  ...
  // Home Wall
  const homeWall = makeWall({ x: 11, z: 11, len: 8, axis: 'z', h: 1.2, fence: true });
  ctx.scene.add(homeWall);
  ctx.collision.addBox(11, 11 + 8/2, 0.2, 8/2, y, y + 1.2, 0, 'Home Wall');
}
