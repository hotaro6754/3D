...
import { bake, trs, rngKit } from '../rendering/util';
...
export function buildSakura(ctx, spots) {
  ...
  woodMat.onBeforeCompile = (shader, renderer) => {
    if (originalWoodOBC) originalWoodOBC.call(woodMat, shader, renderer);
  ...
  const wood = new THREE.Mesh(bake(woodParts), woodMat);
  ...
  blobs.forEach((list, i) => {
    ...
    const mergedGeo = bake(parts);
    
    const mat = cel({ color: BLOB_TONES[i], bands: 'soft', tint: BLOB_TINT[i], cache: false });
    const originalOBC = mat.onBeforeCompile;
    const originalKey = mat.customProgramCacheKey;
    mat.onBeforeCompile = (shader, renderer) => {
      if (originalOBC) originalOBC.call(mat, shader, renderer);
      shader.uniforms.uTime = sakuraUniforms.uTime;
      shader.vertexShader = shader.vertexShader
        .replace('#include <common>', '#include <common>\nuniform float uTime;\n')
        .replace('#include <begin_vertex>', `
...
export function buildGrove(ctx, spots) {
  ...
  blobs.forEach((list, i) => {
    ...
    const parts = list.map(m => ({ geometry: blobGeo, matrix: m }));
    const mergedGeo = bake(parts);
    const mesh = new THREE.Mesh(mergedGeo, cel({ color: GREEN_TONES[i], bands: 3, tint: 0x5b6f8c }));
    mesh.castShadow = true;
...
export function buildCedar(ctx, spots) {
  ...
  tiers.forEach((list, i) => {
    if (!list.length) return;
    const parts = list.map(m => ({ geometry: coneGeo, matrix: m }));
    const mergedGeo = bake(parts);
    const mesh = new THREE.Mesh(mergedGeo, cel({ color: CEDAR_TONES[i], bands: 3, tint: 0x59657f }));
    mesh.castShadow = true;
    // see the header: no canopy in this world receives shadow, and this is the
    // darkest one there is
    mesh.receiveShadow = false;
    mesh.name = 'cedarCanopy' + i;
    ctx.add(mesh);
  });
